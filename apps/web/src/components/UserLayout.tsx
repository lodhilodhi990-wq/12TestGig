...
export default function UserLayout({ children }: { children: React.ReactNode }) {
  ...
  const sections: NavSection[] = [
    ...
    },
    {
      ...
      items: [
        { name: 'Live Support & Help Desk', href: '/tester/support', icon: HelpCircle, badge: '24/7' },
    ...
    <div className="min-h-screen bg-zinc-50 flex flex-col md:flex-row font-sans">
      ...
      `}>
        ...
        <div className="p-3 border-t border-zinc-200 bg-zinc-50/50">
          ...
          <button
            onClick={logout}
            className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-xl text-xs font-semibold text-red-600 hover:bg-red-50 hover:text-red-700 transition cursor-pointer"
          >
        ...
        <header className="h-16 bg-white border-b border-zinc-200 flex items-center justify-between px-4 md:px-8 sticky top-0 z-30">
          <div className="flex items-center gap-3">
            <button 
              ...
              className="p-1.5 rounded-lg text-zinc-600 hover:bg-zinc-100 md:hidden cursor-pointer"
            >
              <Menu className="w-6 h-6" />
            </button>
            <div className="hidden md:flex items-center gap-2 text-xs font-bold text-zinc-600">
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
              <span>Google Play Closed Testing Cloud</span>
            </div>
          </div>
          
          <div className="flex items-center gap-3">
            {/* Quick Buy Coins Link */}
            <Link 
              href="/customer/billing"
              className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-500/10 text-amber-700 hover:bg-amber-500/20 text-xs font-bold transition border border-amber-500/20"
            >
            ...
            <Link 
              href="/tester/support"
              className="p-2 rounded-xl text-zinc-500 hover:text-blue-600 hover:bg-blue-50 transition"
